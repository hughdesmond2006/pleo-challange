...
class CommentInput extends Component {
  ...
  saveChange = () => {
    ...
    if (
      ...
      (this.state.isMessageShowing && !this.state.isMessageSuccess)
    ) {
      this.setState({ text: newText });

      // post new comment to api
      axios
        .post("http://localhost:3000/expenses/" + this.props.id, {
          comment: newText
        })
        .then(res => {
      ...
      <div className={styles.wrap}>
        <div
          className={
            styles.text +
            " " +
            (text === "" && !isEditingEnabled ? styles.textHinting : "") +
        ...
        {isMessageShowing && (
          <div
            className={
              styles.toast +
              " " +
              (isMessageSuccess ? styles.toastSuccess : styles.toastFail)
            }
          >
            {messageText}
          </div>
